...
public sealed class BpuiFileActivationService : IBpuiFileActivationService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            {
                return await FailAsync("Package archive was not found.");
            }
            ...
            if (!result.Success)
            {
                return await FailAsync(result.ErrorMessage ?? I18nHelper.GetLocalizedString("PackageImportFailed"));
            }
    ...
    private async Task<BpuiFileActivationResult> FailAsync(string message)
    {
        await MessageBoxHelper.ShowErrorAsync(
            $"{I18nHelper.GetLocalizedString("PackageImportFailed")}: {message}",

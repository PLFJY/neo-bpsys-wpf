    ...
    ILogger<GitHubDownloadUrlResolver> logger) : IGitHubDownloadUrlResolver
{
    ...
    public async Task<string> ResolveAsync(string url, CancellationToken cancellationToken = default)
    {
        ...
        var cacheKey = $"{preferred}\n{url}";
        lock (_cache) if (_cache.TryGetValue(cacheKey, out var cached)) return string.IsNullOrEmpty(cached) ? url : cached + url;
        ...
        foreach (var mirror in candidates)
        {
            try
            {
                ...
                lock (_cache) _cache[cacheKey] = mirror;
                return mirror + url;
            }
        ...
        lock (_cache) _cache[cacheKey] = string.Empty;
        return url;
    }

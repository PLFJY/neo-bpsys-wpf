...
type Bootstrap = { FullWindowType: string; DisplayName: string; Layout: AnyRecord | null; Resources: Record<string, string>; Diagnostics: string[] }
...
function ImageControl({ name, config, resources, runtime }: { name: string; config: AnyRecord; resources: Record<string, string>; runtime: RuntimeState }) {
  ...
  return <div className="image-control" id={name} style={style}>
    {image ? <img src={image} style={{ width: '100%', height: '100%', objectFit: fit as CSSProperties['objectFit'] }} /> : text(config.BindingPath) ? <div className="binding-image">[{text(config.BindingPath)}]</div> : null}
...
function TextControl({ config, localized, runtime }: { config: AnyRecord; localized: boolean; runtime: RuntimeState }) {
...
function Canvas({ bootstrap, runtime }: { bootstrap: Bootstrap; runtime: RuntimeState }) {
  ...
  return <><style>{fontFaces}</style><div className="viewport"><div className="canvas" style={{ width, height, transform: `scale(${scaleX}, ${scaleY})`, ['--canvas-width' as string]: `${width}px`, ['--canvas-height' as string]: `${height}px`, backgroundImage: background ? `url(${background})` : undefined }}>
    {Object.entries(controls).map(([name, config]) => { const type = text(config.ControlType); if (type === 'Text') return <TextControl key={name} config={config} runtime={runtime} localized={false} />; if (type === 'LocalizedText') return <TextControl key={name} config={config} runtime={runtime} localized />; if (type === 'Image' || type === 'BorderedImage') return <ImageControl key={name} name={name} config={config} resources={bootstrap.Resources} runtime={runtime} />; if (type === 'Rectangle' || type === 'Polygon' || type === 'BackgroundTintRectangle' || type === 'BackgroundTintPolygon') return <Shape key={name} config={config} background={background} resources={bootstrap.Resources} runtime={runtime} />; if (type === 'GameProgressText') return <GameProgress key={name} config={config} runtime={runtime} />; if (type === 'MapNameText') return <MapName key={name} config={config} runtime={runtime} />; if (type === 'TalentTraitDisplay') return <TalentTrait key={name} config={config} runtime={runtime} />; if (type === 'MapV2Display') return <MapV2 key={name} config={config} runtime={runtime} resources={bootstrap.Resources} />; if (type === 'GlobalScoreRow') return <GlobalScoreRow key={name} config={config} runtime={runtime} />; return <Unknown key={name} name={name} config={config} runtime={runtime} /> })}
  </div></div></>
}
function App() {
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      socket.onmessage = event => { try {
        ...
        if (message.type === 'bootstrap.changed') { setRuntime({ values: {}, sequence: 0, generation: 0 }); load(); return }
    ...
    const socket = connect(); return () => { closed = true; if (retry) clearTimeout(retry); socket.close() }

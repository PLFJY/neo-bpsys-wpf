...
export function ImageRenderer({ name, config, runtime, resources, defaultPickingBorderResourceUrl, behaviorSet }: { name: string; config: ImageConfig; runtime: RuntimeState; resources: Record<string, string>; defaultPickingBorderResourceUrl?: string; behaviorSet?: ControlBehaviorSet }) {
  ...
  return <SemanticControlRoot name={name} behaviorGuid={config.BehaviorGuid} className="web-image" attributes={{ 'data-image-semantic-root': '' }} style={rootStyle}>

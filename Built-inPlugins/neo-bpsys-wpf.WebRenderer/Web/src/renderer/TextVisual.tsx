...
export function TextVisual({ config, runtime, children, className, style }: { config: TextStyle; runtime: RuntimeState; children: ReactNode; className?: string; style?: CSSProperties }) {
  ...
  const visual: CSSProperties = { boxSizing: 'border-box', display: 'flex', justifyContent: horizontal(config.HorizontalAlignment), alignItems: vertical(config.VerticalAlignment), textAlign: textAlign(config.TextAlignment), whiteSpace: wrapping(config.TextWrapping), color: color(bound ?? config.Color, '#fff'), fontFamily: fontFamily(config.FontFamily), fontWeight: config.FontWeight, fontSize: typeof config.FontSize === 'number' && config.FontSize > 0 ? config.FontSize : undefined, lineHeight: 'normal', ...style }

...
internal sealed class LlamaCppServerManager : ILlamaCppServerManager, IDisposable
{
    ...
    {
        _assets = assets; _settings = settings; _storage = storage; _logger = logger; _debugLog = debugLog;
    ...
    public async Task StartAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (string.IsNullOrWhiteSpace(executable)) executable = Path.Combine(_storage.LlamaCppRoot, "current", "llama-server.exe");
        ...
        var arguments = new[] { "--model", model, "--mmproj", mmproj, "--host", "127.0.0.1", "--port", _settings.Settings.LlamaServerPort.ToString(), "--ctx-size", "4096", "--n-gpu-layers", "auto", "--flash-attn", "auto", "--parallel", "1", "--no-webui", "--log-file", log, "--threads", _settings.Settings.CpuThreads.ToString() };
        ...
        _process = Process.Start(info) ?? throw new InvalidOperationException("Failed to start llama-server.");
        _process.OutputDataReceived += OnOutputDataReceived;
        _process.ErrorDataReceived += OnErrorDataReceived;
        _process.BeginOutputReadLine();
        _process.BeginErrorReadLine();
        try { _process.PriorityClass = Enum.TryParse<ProcessPriorityClass>(_settings.Settings.ProcessPriority, true, out var priority) ? priority : ProcessPriorityClass.BelowNormal; } catch (Exception ex) { _logger.LogWarning(ex, "Could not set llama-server priority"); }
        Status = "Starting";
        try
        {
    ...
    {
        if (_process is { HasExited: false }) { _debugLog.Write("llama-server", "Stopping managed process..."); _process.Kill(true); _process.WaitForExit(5000); }
        _process?.Dispose(); _process = null; Status = "Stopped"; return Task.CompletedTask;
    }
    ...
    private void OnProcessExit(object? sender, EventArgs e) { try { StopAsync().GetAwaiter().GetResult(); } catch { } }
    public void Dispose() { AppDomain.CurrentDomain.ProcessExit -= OnProcessExit; OnProcessExit(this, EventArgs.Empty); }
}

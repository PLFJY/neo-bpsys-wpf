...
`.bpui v3` 包必须只携带 Designer v3 前台布局、布局资源、manifest 和可选预览/说明，不得包含或覆盖全局 `Config.json`。manifest 使用根级 `MinVersion`，不包含 `App` 对象或 `App.MinVersion`。Canvas Background GUI 和窗口级 `AllowTransparency` 选项已在包标准中定义为后续能力，当前仍属于编辑器基础之后的计划项。

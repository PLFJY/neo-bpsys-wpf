...
本文是前台窗口设计者模式 v3 重构的 Phase 0 设计文档。Phase 0 只描述目标、边界和分阶段计划，不修改运行时行为。
...
`config.json` 中缺失或 `null` 的 `Version` 表示 legacy 配置，应在 Phase 1 触发迁移流程。Phase 0 不实现 `Settings.Version`，也不实现迁移代码。
...
`BackgroundImage` 与控件图片路径的 resolver 是后续实现细节。当前设计只约定默认语义，不在 Phase 0 增加代码。
...
public interface IFrontedControl
{
    ...
    Type ConfigType { get; }
    FrameworkElement Create(string name, object config, FrontedControlContext context);
}
```

`FrontedControlContext` 可在后续设计中承载 `ISharedDataService`、资源 resolver、窗口/Canvas 元信息、服务提供器和日志等上下文。
...
| Phase 1 | 增加 `Settings.Version = 3`，建立 legacy config 迁移 skeleton。 |
| Phase 2 | 增加 v3 layout models、资源 resolver、Text/Image factory、renderer skeleton。 |

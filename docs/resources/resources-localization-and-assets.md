...
v3 窗口标题的包内译名保存在布局 JSON 根级 `DisplayNames`，而不是要求外部包引用宿主资源。新布局优先按当前语言读取该字典，再按 `zh_Hans`、`en_US`、`ja_JP` 顺序回退；缺少该字段的历史内置布局才查询旧的 `Designer.Window.*` 资源。显示名称由导入器执行长度校验，超限包拒绝导入，不静默截断。

...
`WidgetsWindow/MapV2Canvas` 的 `MapV2Display` 已提供地图名、队名、阵营文字三组字体/颜色/字号配置，以及 `PickingBorderImagePath`、`PickingBorderFillColor`。旧 `MapBpV2_*` 文本样式和 picking border 图片/颜色可以写入这些 v3 字段，不再需要提示“MapV2Display 无 v3 image/color config”。

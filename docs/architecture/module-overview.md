...
| `OcrService` / `PaddleRuntime` | 模块内服务，PaddleOCR 模型下载、删除、切换、推理和失败重建；Paddle CPU/CUDA native runtime 由模块管理，CUDA Toolkit 是系统级 NVIDIA 前置条件 | CPU 固定随模块发布于 `Runtime/Paddle/cpu/`；Paddle CUDA native 包也位于当前模块根目录，因此 Debug 模块与正式安装模块各自维护一份。系统 CUDA 11.8/cuDNN 8.9.6 由所有模块实例共享：程序通过 UAC 打开 CUDA 图形安装程序，再以隐藏的提权 PowerShell 把已静态链接 zlib 的 cuDNN DLL 安装到 CUDA 的系统 `bin`、写入明确版本标记并确保该目录在系统 PATH；CUDA/cuDNN 安装包缓存在模块的 `Runtime/Paddle/Downloads/NVIDIA/`，重试时先校验哈希并复用，不把 CUDA/cuDNN DLL 写入主程序目录 |

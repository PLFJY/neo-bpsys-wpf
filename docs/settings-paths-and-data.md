...
Phase 9B.1 起，`IFrontedLayoutPackageManager` 会读取 `%APPDATA%\neo-bpsys-wpf\FrontedLayoutPackages`，始终列出虚拟 `builtin` 包，跳过保留的 `local` 目录，并读取普通已安装包目录下的 `manifest.json`。缺少或损坏 manifest 的包会以校验错误显示，不会让管理器崩溃。`active-package.json` 缺失时默认视为 `builtin` 活动；当前激活实现只写入/清除活动包状态，为后续真正复制布局和热切换做准备。

Phase 9C 起，`FrontManagePage` 的 Layout Packages 页支持导出 v3 `.bpui` 包。导出会从 `IFrontedLayoutService` 按“用户布局优先、内置兜底”加载已迁移前台布局，生成 `manifest.json`、`layouts/` 和 `resources/`，但不会包含全局 `%APPDATA%\neo-bpsys-wpf\Config.json`，也不会包含 legacy `CustomUi/` 或 `FrontElementsConfig/`。导出不会默认安装生成的包，因此通常不会刷新 `%APPDATA%\neo-bpsys-wpf\FrontedLayoutPackages` 下的已安装包列表。

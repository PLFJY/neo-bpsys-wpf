...
Phase 13B 已实现第三方插件前台控件的 core registry/runtime plumbing：布局 JSON 可读取 `plugin:*` 控件并保留插件专属属性，已安装插件可通过 descriptor/contributor API 注册运行时控件，缺失插件控件在前台 runtime 中跳过并记录 warning。本节仍只定义 `.bpui` 依赖 schema 和导入策略；Designer Add Control 插件 UI、`.bpui` 依赖扫描/导入导出和插件市场安装流程分别留到 Phase 13C、13D、13E。
...
4. 合并依赖列表。
5. 检查已安装插件及版本。
6. 分类为：已安装且版本满足、已安装但版本过低、未安装但插件市场可用、未安装且市场找不到。
...
5. 如果插件市场找不到插件，应提示用户“强制导入并删除缺失插件控件”或“取消导入”。
...
| Phase 13C | Designer 插件控件支持，包括 Add Control、属性元数据和缺失插件占位符。 |

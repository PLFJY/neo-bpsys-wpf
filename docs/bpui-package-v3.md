...
Phase 13B 已实现第三方插件前台控件的 core registry/runtime plumbing：布局 JSON 可读取 `plugin:*` 控件并保留插件专属属性，已安装插件可通过 descriptor/contributor API 注册运行时控件，缺失插件控件在前台 runtime 中跳过并记录 warning。Phase 13C 已实现 Designer Add Control 插件 UI、插件声明式属性编辑、Canvas `RequiredPlugins` 同步和缺失插件 Designer 占位符。本节仍只定义 `.bpui` 依赖 schema 和导入策略；`.bpui` 依赖扫描/导入导出和插件市场安装流程分别留到 Phase 13D、13E。

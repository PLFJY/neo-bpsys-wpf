...
绝对路径只应作为编辑时临时输入。Phase 9B.0 的 Canvas Properties GUI 在用户选择本地背景图片后，会复制文件到本地资源目录，并在 layout JSON 中写入 `bpui://local/...`。导出 `.bpui v3` 时再把引用到的 `local` 资源复制进导出包，并重写为 `bpui://{PackageId}/...`。

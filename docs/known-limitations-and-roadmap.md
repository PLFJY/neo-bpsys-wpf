...
Fronted Designer v3 当前已完成基础设施阶段，并将 `ScoreSurWindow`、`ScoreHunWindow`、`ScoreGlobalWindow` 和 `CutSceneWindow` 接入 v3 renderer；这些窗口的内置默认布局比分文本已绑定 `CurrentGame.MatchScore`。`CutSceneWindow` 默认布局使用 `TalentTraitDisplay`、`GameProgressText`、`MapNameText` 封装天赋/辅助特质显示、BO3/BO5 进度文本和地图名本地化，不再在 XAML 中硬编码这些业务显示逻辑。大多数前台窗口仍是 XAML-first，包括 `BpWindow`、`GameDataWindow` 和 `WidgetsWindow`。旧设计者模式、旧 `config.json` 前台设置和旧 XAML-first 运行时仍保留。
...
| 边界 | 说明 |
| --- | --- |
| `Team.Score` 语义混杂 | 当前仍被 `BpWindow`、`GameDataWindow`、`WidgetsWindow` 等旧 XAML-first 窗口使用；迁移期不要立即删除。新服务中的同步只是 transitional compatibility mirror，不是权威状态，Score 系列默认 v3 布局、CutScene v3 默认布局和后台 ScorePage 已不再依赖它。 |

...
16. **Phase 13C 已实现 Designer 插件前台控件支持**：插件 `ControlType` 标准为 `plugin:<PackageId>/<ControlTypeName>`，Canvas 可声明并由编辑器同步 `RequiredPlugins`，`.bpui` manifest 可汇总 `PluginDependencies`。代码已支持 parser、`RequiredPlugins` 模型、`PluginFrontedControlConfig` extension-data roundtrip、descriptor/contributor registry、已安装插件控件 runtime 创建、Designer Add Control 插件目录、插件声明式 Property Grid 元数据、缺失插件控件 Designer 占位符，以及缺失插件控件运行时跳过并 warning。当前不要误判为已实现 `.bpui` 依赖扫描/导入导出或插件市场自动安装；这些仍是 Phase 13D、13E。

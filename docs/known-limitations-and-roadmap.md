...
16. **Phase 13B 已实现插件前台控件 registry/runtime plumbing**：插件 `ControlType` 标准为 `plugin:<PackageId>/<ControlTypeName>`，Canvas 可声明 `RequiredPlugins`，`.bpui` manifest 可汇总 `PluginDependencies`。代码已支持 parser、`RequiredPlugins` 模型、`PluginFrontedControlConfig` extension-data roundtrip、descriptor/contributor registry、已安装插件控件 runtime 创建，以及缺失插件控件运行时跳过并 warning。当前不要误判为已实现 Designer Add Control 插件 UI、Property Grid 插件元数据渲染、`.bpui` 依赖扫描/导入导出或插件市场自动安装；这些仍是 Phase 13C、13D、13E。
...
| Phase 13C | Designer 插件控件支持，包括 Add Control、属性元数据、MissingPlugin 占位符和安装引导。 |

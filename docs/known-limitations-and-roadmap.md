...
16. **Phase 13D 已实现 Designer 插件前台控件 `.bpui` 依赖扫描**：插件 `ControlType` 标准为 `plugin:<PackageId>/<ControlTypeName>`，Canvas 可声明并由编辑器/导出器同步 `RequiredPlugins`，`.bpui` manifest 会汇总 `PluginDependencies`。导入器会合并 manifest、Canvas 和实际控件扫描结果，缺失插件控件默认阻止导入并要求用户选择；强制导入会删除缺失插件控件、保留其他控件、更新 Canvas `RequiredPlugins`，且不修改原始 `.bpui`。`.bpui` 仍不得包含插件 DLL 或插件 zip，也不会静默下载、安装或加载插件。插件市场自动安装/更新引导仍是 Phase 13E。

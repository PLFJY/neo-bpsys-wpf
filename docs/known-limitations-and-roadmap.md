...
8. SettingPage 中现有 `.bpui` 导入/导出仍是 legacy/deprecated 流程，会处理 `Config.json`、`CustomUi/` 和 `FrontElementsConfig`，暂时保留；FrontManagePage 的 legacy 转换不会调用该旧导入流程，也不会覆盖全局 `Config.json`。

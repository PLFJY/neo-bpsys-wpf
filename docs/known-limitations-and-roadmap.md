...
BP 识别路径读取 SmartBP 模块资源中的 `BpRecognitionLayoutProfile.json` 粗裁剪配置，六个区域同时服务 OCR 与实验 AI 策略。用户可通过通用 `RegionEditorWindow` 在当前捕获帧或内置测试图上可视化调整全部区域，结果保存为 AppData 下的 SmartBP recognition profile 覆盖；运行时与编辑器读取同一份 `TopLeftStatus` 配置。自动循环先对该区域做确定性本地 OCR，以三个区域选择/等待开始标题为强锚点；命中后覆盖不稳定的 Business AI 阶段、锁存 post-BP 状态，并在内容识别前进入队列排空停止。这套配置独立于 PaddleOCR 赛后数据 OCR 细区域。Tesseract traineddata、本地视觉模型、llama.cpp runtime 和长日志均存放在 SmartBP 模块目录；本地视觉模型列表、llama.cpp runtime manifest、AI prompt、内置测试帧、OCR 别名和 SmartBP 默认区域配置均随 SmartBP 模块 `Resources` 复制到模块输出目录，不放在主程序 `Resources`。AI + AI OCR 可按业务 AI / AI OCR 角色启动单个复用服务器或双 llama-server 进程；每个角色使用独立端口、状态文件和日志文件。llama.cpp 运行时可从内置 runtime manifest 在线安装，也可通过用户配置的远程 manifest API 检查更新，并存放于 `AI/LlamaCpp/Runtimes/{runtimeId}`。AppData 中只保存纯配置（如 `SmartBp/RecognitionSettings.json`、`SmartBp/BpRecognitionLayoutProfile.json`、托管 llama-server 进程状态）。默认仍为识别预览；只有用户显式启用自动应用后，才会通过角色选择服务应用已解析候选。不识别 MapBP。

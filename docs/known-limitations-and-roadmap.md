...
| 本地视觉模型 + llama.cpp BP 状态识别 | 保留为实验策略；当前不作为默认路径 |
...
自动循环通过细分场景门禁限制写入：只在角色 BP 中应用角色操作，天赋阶段只同步引导，区域选择、等待开始、加载和对局内会丢弃延迟结果并暂停。区域选择和 MapBP 均不在此识别范围内。本地视觉模型 manifest 支持 HuggingFace/中文镜像、ModelScope 直链以及独立、内嵌或无 mmproj 三种显式模式，并用角色标记区分业务 VLM 与 AI OCR 文本提取模型。
...
BP 识别路径读取 SmartBP 模块资源中的 `BpRecognitionLayoutProfile.json` 粗裁剪配置，七个区域同时服务 OCR 与实验 AI 策略。用户可通过通用 `RegionEditorWindow` 在当前捕获帧或内置测试图上可视化调整全部区域，结果保存为 AppData 下的 SmartBP recognition profile 覆盖；运行时与编辑器读取同一份配置。自动循环每 tick 先 OCR `TopCenterStatus`，用归一化、编辑距离和关键词覆盖评分识别阵营选择、双方天赋调整及即将进入区域选择；未知结果只跳过本 tick，弱 transition 需连续两次确认，强 transition 会立即阻止新内容识别并进入排空停止。`TopLeftStatus` 保留为稍晚出现的区域选择/等待开始硬确认，两种区域不会合并使用。post-BP latch 设置后，后续阶段波动不会恢复内容 OCR、AI OCR、业务 AI 融合、字段合并或候选操作生成。这套配置独立于 PaddleOCR 赛后数据 OCR 细区域。Tesseract traineddata、本地视觉模型、llama.cpp runtime 和长日志均存放在 SmartBP 模块目录；本地视觉模型列表、llama.cpp runtime manifest、AI prompt、内置测试帧、OCR 别名和 SmartBP 默认区域配置均随 SmartBP 模块 `Resources` 复制到模块输出目录，不放在主程序 `Resources`。AI + AI OCR 可按业务 AI / AI OCR 角色启动单个复用服务器或双 llama-server 进程；每个角色使用独立端口、状态文件和日志文件。llama.cpp 运行时可从内置 runtime manifest 在线安装，也可通过用户配置的远程 manifest API 检查更新，并存放于 `AI/LlamaCpp/Runtimes/{runtimeId}`。AppData 中只保存纯配置（如 `SmartBp/RecognitionSettings.json`、`SmartBp/BpRecognitionLayoutProfile.json`、托管 llama-server 进程状态）。默认仍为识别预览；只有用户显式启用自动应用后，才会通过角色选择服务应用已解析候选。不识别 MapBP。

...
16. **Phase 13A 只是插件前台控件的文档和 schema 规划**：插件 `ControlType` 标准为 `plugin:<PackageId>/<ControlTypeName>`，Canvas 可声明 `RequiredPlugins`，`.bpui` manifest 可汇总 `PluginDependencies`。当前不要误判为已实现完整插件控件 runtime、Designer Add Control 插件 UI、`.bpui` 依赖导入导出或插件市场自动安装。缺失插件控件的目标策略是运行时跳过并 warning、Designer 可显示 MissingPlugin 占位符、强制导入删除缺失插件控件。
